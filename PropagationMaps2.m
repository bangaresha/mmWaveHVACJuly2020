...
mesh_.xNodeNum = 10;
mesh_.yNodeNum = 10;
structImage1 = structImage(1:25, 1:50);
structImage2 = structImage(476:500, 1:50);
structImage = [structImage1; structImage2];
...
Rx.TotalRSSI = 10*log10(abs(RxReflect(1,:)));
Rx.TotalRSSI(find(isinf(Rx.TotalRSSI) == 1)) = 0;
...
Rx.TotalRSSI = 10*log10(abs(RxReflect(2,:)));
Rx.TotalRSSI(find(isinf(Rx.TotalRSSI) == 1)) = 0;
Rx.TotalRSSI(find((Rx.TotalRSSI) == 0)) = min(min(Rx.TotalRSSI));
imageRSSI = zeros(mesh_.yNodeNum,mesh_.xNodeNum,3);
if mesh_.zNodeNum ~= 1
...
Rx.TotalRSSI = 10*log10(abs(Rx.LosRssi + (reflectExaggerationFac * Rx.ReflecRssi(1,:)) + (reflectExaggerationFac *...
    Rx.ReflecRssi(2,:)) ));
imageRSSI = zeros(mesh_.yNodeNum,mesh_.xNodeNum,3);
if mesh_.zNodeNum ~= 1
    zplaneHeight =  linspace(boundary(3,1),boundary(3,2),mesh_.zNodeNum);
